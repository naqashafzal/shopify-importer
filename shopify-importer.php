...
final class Shopify_Importer_Plugin {
    ...
    private function define_constants() {
        define( 'STW_IMPORTER_VERSION', '1.3.4' );
    }
...
class STW_Admin_Menu {
            ...
            <div id="stw-product-section">
                ...
                <?php if (!empty($options['key']) && !empty($options['pass'])): ?>
                    ...
                    <div id="stw-product-list-container" style="display:none;">
                        <h3>Available Products (<span id="product-count">0</span>)</h3>
                        <p>Select the products you wish to import. Collections will be imported as categories.</p>
                        <div id="stw-filter-controls" style="margin-bottom: 15px;"><input type="text" id="stw-product-search" placeholder="Search by product name..." style="width: 300px;"></div>
                        ...
                        <button id="stw-import-selected" class="button button-primary">Import Selected Products</button>
                    </div>
                <?php else: ?><p>Please save your API settings above to enable product fetching.</p><?php endif; ?>
            </div>
            <div id="stw-progress-container" style="display:none;">
                <h2 id="stw-import-status">Starting import...</h2>
    ...
    private function add_inline_styles() { /* CSS for admin page */ ?>
        <style type="text/css">
            ...
            #stw-progress-container { display: none; margin-top: 20px; border: 1px solid #ccd0d4; padding: 15px; border-radius: 4px; background-color: #fff; }
    ...
    private function add_inline_js() { /* JavaScript for admin page */ ?>
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                ...
                const searchInput = $('#stw-product-search'), paginationControls = $('#stw-pagination-controls');
                ...
                fetchBtn.on('click', function() {
                    ...
                    $.post(ajaxurl, { action: 'stw_fetch_products', nonce: '<?php echo wp_create_nonce("stw_import_ajax_nonce"); ?>' }, function(response) {
                        ...
                        if (response.success) {
                            allProductsData = response.data.products; currentPage = 1; renderProductList(); productListContainer.show();
                ...
                function renderProductList() {
                    ...
                    productsToShow.forEach(function(p) {
                        productListBody.append(`<tr><th scope="row" class="check-column"><input type="checkbox" name="products[]" value="${p.id}"></th><td>${p.title}</td><td>${p.vendor || 'N/A'}</td><td>${p.tags || 'N/A'}</td></tr>`);
                    });
                    if (totalPages > 1) {
                ...
                $('#cb-select-all').on('click', function() { productListBody.find('input[type="checkbox"]').prop('checked', this.checked); });

                importBtn.on('click', function() {
                    const selectedIds = productListBody.find('input[type="checkbox"]:checked').map(function() { return $(this).val(); }).get();
        ...
        foreach ($endpoints as $endpoint) {
            ...
            if (!is_wp_error($response)) {
                ...
                if (isset($data[$endpoint])) {
                    $all_collections = array_merge($all_collections, wp_list_pluck($data[$endpoint], 'title'));
                }
            }
        }
